...
/**
 * This is the model implementation of GoogleAnalyticsUpdateCustomMetric.
 * 
 *
 * @author 
 */
public class GoogleAnalyticsUpdateCustomMetricNodeModel extends NodeModel {
    
    
	GoogleAnalyticsUpdateCustomMetricConfig config = new GoogleAnalyticsUpdateCustomMetricConfig();
	
	
    protected GoogleAnalyticsUpdateCustomMetricNodeModel() {
    ...
    protected PortObject[] execute(PortObject[] inObjects, ExecutionContext exec)
    	...
    	cells.add(new StringCell(config.getCm_id()));
		cells.add(new StringCell(config.getCm_name()));
		cells.add(new StringCell(config.getCm_scope()));
		cells.add(new StringCell(config.getCm_format()));
		cells.add(new StringCell(config.getCm_min()));
		cells.add(new StringCell(config.getCm_max()));

		CustomMetric body = new CustomMetric();
		body.setName(config.getCm_name());
		body.setScope(config.getCm_scope());
		body.setActive(config.getCm_active());
		body.setType(config.getCm_format());
		body.setMaxValue(config.getCm_max());
		body.setMinValue(config.getCm_min());
		
				
		try {
    	
        	managementClient.updateCustomeMetric(config.getCm_id(), body);
    ...
    private DataTableSpec createSpec() {
    	...
    	colSpecs.add(new DataColumnSpecCreator("Custom Metric Name", StringCell.TYPE).createSpec());
    	colSpecs.add(new DataColumnSpecCreator("Custom Metric Scope", StringCell.TYPE).createSpec());
    	colSpecs.add(new DataColumnSpecCreator("Custom Metric Format", StringCell.TYPE).createSpec());
    	colSpecs.add(new DataColumnSpecCreator("Custom Metric Min", StringCell.TYPE).createSpec());
    	colSpecs.add(new DataColumnSpecCreator("Custom Metric Max", StringCell.TYPE).createSpec());
